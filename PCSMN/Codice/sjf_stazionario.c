...
struct req {
    double size;            //indica la size della richiesta
    ...
    while (!finish) {
        ...
        **/
        ...
        if (t.current == t.arrival) {
            ...
            job_list->fog_id = i;
            //genero il tempo di servizio necessario per quella richiesta e lo memorizzo
            ...
            if (p > P) {
                ...
                cloud.number++;
                //individuo nella lista di job quella relativa alla richiesta corrente
                for (j = job_list; j != NULL; j = j->next)
                    if(j->completion_fog == t.current)
                        break;
                 //genero il tempo di servizio necessario per quella richiesta per il cloud e lo memorizzo
